...
### Markdown 清理与一键整理（`mdclean` / `tidy`）

从网上粘来的文字最常见的两种脏：**Markdown 标记**和**多余空格/空白行**。

```bash
python -m wordfactory.cli mdclean "某文档.docx" --dry-run     # 清 Markdown 标记（# 标题、**粗体**、`代码`、- 列表）
python -m wordfactory.cli tidy    "某文档.docx" --dry-run     # 去段尾空格 + 压缩无意义空白行
```

`mdclean` 照参考宏 `MarkDown语言清除.bas`（§2.1）：`` `X` `` → `“X”`、`**X**`/`*X*` 去星号、
段首 `#` 全剥、剥一个列表标记、段内星号删掉、连续空格折叠、逐段 Trim。
**与宏不同的一点**：宏是整段回写（`selectedRange.text = …`，会把整段格式压平），
我们**逐条规则做局部替换** —— 只动匹配到的那一小段，其余文字的 run 格式原样保留
（有单测钉住：`**粗体**` 处理完仍然加粗，run 数量不变）。

`tidy` 的默认口径是**保守**的（与你选的口径一致）：只删"明显无意义"的 ——
段尾空格、连续空白段（压成一个）、文档首尾的空白段。**默认不动**：
段首空格（可能是用空格做的缩进）与段内连续空格（会把表题的空格居中压掉）；
后者一旦开（`--collapse-space-runs`）会**自动跳过题注段落**。

> 实测你的报告：`tidy` 默认命中 **42 处**（段尾空格 12、空白段压缩 28、首尾空白段删除 2）；
> `mdclean` 命中 0（这份报告里没有 Markdown 标记）。
> **`Copy++.exe` 是第三方闭源程序，我没有反编译它** —— 这里按"行为"实现；
> 它还有哪些行为你需要，给我一段"处理前/处理后"的样例，我照着补齐。
